...
export class KeywordCaptioner {
  constructor(
    private readonly options: InstantCaptionConfig,
  ...
  private fromLatin(raw: string): string | null {
  ...
  private fromCjk(raw: string): string | null {
    ...
    for (const token of tokens) {
      if (/[A-Za-z]/.test(token)) {
        ...
        if (CJK_FUNCTION_TOKENS.has(cleaned)) continue
        kept.push(cleaned)
      }
    }
    if (kept.length === 0) return null
...
function stripOpeners(text: string, openers: readonly string[]): string {
  let out = text
  let changed = true
  while (changed) {
    changed = false
 ...
 */
function assembleCjk(kept: readonly string[], maxChars: number): string {
  let out = ''
  for (const piece of kept) {
    const next = out === '' ? piece : `${out} ${piece}`
    if ([...next].length > maxChars) {
      if (/[A-Za-z]/.test(piece)) break
      const room = maxChars - [...out].length
      if (room > 0) out += [...piece].slice(0, room).join('')
      break
    }
    out = next
  }
