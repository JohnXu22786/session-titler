 ...
 */
import { BlockAssembler, createUserMessage, type FinishReason, type LlmService } from '@deepseek-ai/dsh-llm'
import type {
...
export class Refiner {
  constructor(
    ...
    private readonly config: RefineCaptionConfig,
    private readonly summary: SummaryConfig,
    private readonly router: BudgetRouter,
    private readonly normalizer: CaptionNormalizer,
    private readonly log: RefinerLog,
  ) {}
   ...
   */
  async refine(request: SessionTitleProviderRequest): Promise<RefinedCaption> {
    request.signal.throwIfAborted()
    const { session, messages, route } = request
    const selected = selectWithinBudget(messages, this.config.maxInputBytes)
    if (selected.length === 0) throw new CaptionSkippedError('no source messages within budget')

    const target = await this.router.resolve(route)
    if (target === undefined) throw new CaptionSkippedError('no model route available')
    request.signal.throwIfAborted()
    ...
    if (title.length === 0) throw new CaptionSkippedError('refiner produced no caption text')
  ...
  private systemPrompt(): string {
    ...
    if (this.summary.enabled) {
      lines.push(
        `- On the second line output a one-sentence session summary (at most`,
        `  ${this.summary.maxChars} characters) for list display. No other lines.`,
      )
    } else {
  ...
  private throwForFinish(finish: FinishReason, signal: AbortSignal): void {
    ...
    switch (finish.kind) {
      ...
      case 'tool-calls':
        throw new Error('session-caption: refiner unexpectedly requested a tool')
      default:
        throw new Error(`session-caption: unsupported finish reason "${String((finish as { kind?: unknown }).kind)}"`)
    }
  }
}
...
export function selectWithinBudget(
  ...
  for (let i = messages.length - 1; i >= 0; i--) {
    ...
    if (selected.length > 0 && bytes + width > maxBytes) break
...
export function frameMessages(selected: readonly SessionTitleUserMessage[]): string {
  return (
    'Caption this session from the JSON array of human messages below:\n' +
...
export function splitLines(text: string): { title: string; note: string } {
  const lines = text.split(/\r?\n/)
  const first = lines.findIndex((line) => line.trim().length > 0)
  if (first < 0) return { title: '', note: '' }
